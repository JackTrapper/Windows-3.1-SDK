...
#include "ddemlsv.h"		     /* specific to this program	      */
#include "huge.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
    ...
    int             nCmdShow;           /* Param for first ShowWindow() call. */
{
    ...
    if (!DdeInitialize(&idInst, (PFNCALLBACK)MakeProcInstance((FARPROC)DdeCallback,
                hInstance), APPCMD_FILTERINITS, NULL)) {
        Hszize();
...
LPARAM lParam;				    /* additional information	       */
{
    ...
    case WM_INITMENU:
        ...
        CheckMenuItem(wParam, IDM_BLOCKALLCBS,
                fAllBlocked ? MF_CHECKED : MF_UNCHECKED);
        CheckMenuItem(wParam, IDM_UNBLOCKALLCBS,
                fAllEnabled ? MF_CHECKED : MF_UNCHECKED);
        CheckMenuItem(wParam, IDM_BLOCKNEXTCB,
                fBlockNextCB ? MF_CHECKED : MF_UNCHECKED);
        CheckMenuItem(wParam, IDM_TERMNEXTCB,
                fTermNextCB ? MF_CHECKED : MF_UNCHECKED);
        CheckMenuItem(wParam, IDM_RUNAWAY,
                cRunaway ? MF_CHECKED : MF_UNCHECKED);
        CheckMenuItem(wParam, IDM_APPOWNED,
    ...
    case WM_COMMAND:           /* message: command from application menu */
        ...
        case IDM_RENDERDELAY:
            DoDialog("VALUEENTRY", RenderDelayDlgProc, 0, TRUE);
            InvalidateRect(hwndServer, &rcRndrDelay, TRUE);
            break;

        case IDM_SETSERVER:
            DoDialog("VALUEENTRY", SetServerDlgProc, 0, TRUE);
            break;

        case IDM_SETTOPIC:
            DoDialog("VALUEENTRY", SetTopicDlgProc, 0, TRUE);
            break;

        case IDM_CONTEXT:
            DoDialog("CONTEXT", ContextDlgProc, 0, TRUE);
            break;
    ...
    case WM_DESTROY:                  /* message: window being destroyed */
        if (fAppowned) 
            SendMessage(hwndServer, WM_COMMAND, IDM_APPOWNED, 0);
...
HWND hwnd)
{
    ...
    wsprintf(szT, "Items available on the %s topic: Count, Rand, Huge.", (LPSTR)szTopic);
    rcTopics = rc;
