...

{
     ...
     /* Don't take on displaying more than 256 colors */
     if (iNumColors > PALETTESIZE) {
	 if ( LoadString (hInstance, IDSCANTLOAD,
			  szCantLoad, sizeof (szCantLoad)))
	     MessageBox (NULL, szCantLoad, szAppName, MB_OK|MB_ICONEXCLAMATION);
	 return FALSE;
     }
...

{
    ...
    switch (iMessage){
	 ...
	 case WM_PAINT:
	      ...
	      for (nStart = iLoop = 0; iLoop < (WORD)iNumColors; iLoop++){
